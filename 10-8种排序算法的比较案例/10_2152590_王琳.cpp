...
void SortCompareSystem<T>::quickSort(T tempArray[], int left, int right)
{
	...
	compareNumber += 3;
	int middle = (left + right) / 2; //中位数
	//两两进行比较，将左端点和右端点以及中位数进行比较
	//按从小到大的顺序排列，取中值数作为基准
	...
	if (left + 2 <= right)
	{
		...
		while (true)
		{
			//根据pivot进行划分，符合左小右大的规律循环继续往下比较，注意!没有等于号
			...
			compareNumber++;
			if (i < j)			//不符合左小右大，将两个结束循环的位置进行交换，然后从该位置继续往后比较
			{
		...
		//swapNumber(tempArray[j], tempArray[left]);，因为他把中值数放在了第一个，所以就不需要再交换了
		quickSort(tempArray, left, j);		//左序列继续划分
		quickSort(tempArray, j + 1, right); //右序列继续划分
	}
...
void SortCompareSystem<T>::heapSort(T tempArray[], int length)
{
	...
	{
		//将最大堆的第一个对象V[0]与最后一个元素对调
		swapNumber(tempArray[0], tempArray[i]);
		//对前面的n-1个对象使用堆的调整算法，重新建立最大堆
...
template<typename T>
void SortCompareSystem<T>::sortCompareSystem()
{
	int number = 0;	//要产生的随机数个数
	int choice = 0;
	string sortName = ""; //用于最后输统一化
	clock_t start = 0, end = 0;	//用于计时
	cout << "请输入要产生的随机数个数：";
	cin >> number;
	//错误处理，增强程序的健壮性
...
int main()
{
	...
	sortCompareSystem.sortCompareSystem();
	system("pause");
	return 0;
}